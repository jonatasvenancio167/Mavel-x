        ...
        <>
            <ScrollAnimation animateIn='fadeIn'>
                <div className={styles.background}>
                    ...
                    {data?.map(item => <p key={item.id} item={item} className={styles.paragraphFeature}> { item.description } </p>)}
                </div>
                    ...
                    <section className={styles.date}>
                        <table>
                            <tbody>
                                <tr>
                                    <th>Inicio</th>
                                    {data?.map( item => <td key={item.id} item={item}> { item.start ? format(parseISO(item.start), 'd MMM yyy', {locale: ptBR}) : 'Dado não fornecido' } </td> )}
                                </tr>
                                <tr>
                                    <th>Fim</th>
                                    {data?.map(item => <td key={item.id} item={item}> { item.end ? format(parseISO(item.end), 'd MMM yyy', {locale: ptBR}) : 'Dado não fornecido' } </td> )}
                                </tr>
                                <tr>
                                    <th>Modificado</th>
                                    {data?.map(item => <td key={item.id} item={item}> { item.end ? format(parseISO(item.modified), 'd MMM yyy', {locale: ptBR}) : 'Dado não fornecido' } </td> )}
                                </tr>
                            </tbody>
                        </table>
                    </section>
